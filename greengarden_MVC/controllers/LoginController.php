        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['button_login'])) {
                ...
                if ($loginResult > 0) {
                    ...
                    if ($Pseudonyme != $loginResult[0]['Login']) { // Vérifie si le pseudonyme entré ne correspond pas à celui dans la base de données
                        $MsgErr = "Ce pseudonyme n'est pas valide"; // Message pour dire que le pseudonyme n'existe pas

                    } elseif (!password_verify($mot_de_passe, $loginResult[0]['Password'])) { // Vérifie si le mot de passe lié au pseudonyme existe
                        ...
                        $_SESSION['user_type'] = $loginResult[0]['Id_UserType'];
                        header("Location: ./index.php"); // On redirige l'utilisateur vers la page "produits.php" après la connexion réussie
